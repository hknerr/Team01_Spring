...
public class librarypuzzle4 : MonoBehaviour
{
    ...
    private void OnMouseDown()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            if (newItem.text == "Statue 3")
            {

                theControllerScript.deletePosition += 1;
                newItem.text = "";
                theControllerScript.ItemsToDelete[theControllerScript.deletePosition - 1] = "Statue 3";
                Destroy(gameObject);
            }

            else
            {
                Debug.Log("find statue 3");
            }
